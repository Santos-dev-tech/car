 ...
 */
/**
 * How many introducers are on the register, and how many joined today.
 *
 * Public, and aggregate ONLY. A running total is a scoreboard for whoever is signing
 * people up at a bazaar and tells a broker deciding whether to bother that he would not
 * be the first. A list of names and numbers would be a directory of other people's
 * business contacts, which is the one thing this register must never become.
 */
route('GET', '/api/broker/count', () => {
  const today = new Date().toISOString().slice(0, 10);
  return {
    total: (get("SELECT COUNT(*) n FROM users WHERE role='broker'") || {}).n || 0,
    today: (get("SELECT COUNT(*) n FROM users WHERE role='broker' AND substr(created_at,1,10)=?", [today]) || {}).n || 0,
    /* Where the QR should point. Read from settings rather than from the request, so a
       code generated on a laptop on localhost still sends a phone to the real site. */
    joinUrl: (getSetting('public_url', '') || '').replace(/\/$/, '') + '/#/join',
  };
});
