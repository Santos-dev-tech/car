 ...
 */
function futureValue(price, make, years = 3, currentAge = 0, spec = {}) {
  if (!price) return null;
  const projected = market.resale(price, make, years, currentAge, spec);
  ...
  return {
    ...
    retentionPerYear: projected.retentionPerYear,
    basis: projected.basis,
    note: projected.basis.variantAware
      ? `A projection from how this exact ${projected.make} — its body, fuel and engine, not just the badge — holds value in Kenya, with a margin taken off. A dealership offering a buyback should quote at or below the floor.`
      : `A projection from how ${projected.make} holds value in Kenya, with a margin taken off. A dealership offering a buyback should quote at or below the floor.`,
  };
}
