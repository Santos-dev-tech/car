...
function verificationFor(dealerId, brokerId) {
  ...
  const status = suspended ? 'suspended' : allDone ? 'verified' : profile ? 'pending' : 'unregistered';
  ...
  return {
    ...
    /* Written out because "verified" means nothing unless a dealer knows what was
       actually checked. A badge with no stated basis is just a colour. */
    basis: `Verified means: identity documents on file, ${NEEDS_REFERENCES} independent dealerships vouching, and ${NEEDS_FUNDED_DEALS} deals funded through MotoKE under this name.`,
  };
}
