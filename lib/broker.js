...
function registerClient({ dealerId, brokerId, name, phone, note }) {
  ...
  /* A suspended broker cannot take on new clients. Claims they already hold stand — those
     were earned before whatever went wrong — but handing them fresh customers while a
     dealership has an open complaint against them is the opposite of what the register
     is for. */
  const prof = get('SELECT suspended FROM broker_profiles WHERE broker_id=?', [brokerId]);
  if (prof && prof.suspended) {
    return { ok: false, reason: 'Your account is suspended. Speak to the dealership before registering more clients.' };
  }
  ...
  if (existing) {
    const dead = new Date(existing.claim_expires).getTime() < Date.now();

    /* AN EXPIRED CLAIM RELEASES THE NUMBER. The first version refused on the mere
       existence of a row, which meant a claim that died six months ago locked that
       customer away from every broker for good, including the one who registered them.
       Numbers would have silently accumulated as unusable — a slow leak nobody would
       have noticed until a broker could not register a client and had no idea why. */
    if (dead) {
      run(
        `UPDATE broker_clients SET broker_id=?, name=?, note=?, claim_expires=?,
                confirmed_by_client=0, created_at=?
          WHERE id=?`,
        [
          brokerId,
          String(name || '').trim(),
          note ? String(note).trim() : null,
          new Date(Date.now() + CLAIM_DAYS * 864e5).toISOString(),
      ...
      return {
        ok: true,
        renewed: true,
        claim: get('SELECT * FROM broker_clients WHERE id=?', [existing.id]),
      };
    }
...
function bookFor(dealerId, brokerId) {
  ...
  /* Every check this broker has run.
     A broker checks ten people a month. Six weeks later one of them rings back and he is
     starting from nothing: what did they earn, what could they afford, which lender said
     yes. The answer was worked out at the time and saved; it was simply never shown back
     to him. Without this the tool is a calculator you use once and forget, rather than a
     book of clients. */
  const checks = all(
    `SELECT id, ref, name, phone, target_price, deposit, tenor_months,
            approved_count, best_monthly, created_at
       FROM prequalifications
      WHERE dealer_id=? AND introduced_by=?
      ORDER BY created_at DESC`,
    [dealerId, brokerId]
  );

  return { clients, applications: apps, leads, checks };
}

/**
 * Everything this broker holds on one phone number, gathered for the moment a client
 * rings back. Scoped to the broker in SQL, never filtered afterwards.
 */
function historyFor(dealerId, brokerId, phone) {
  const p = key(phone);
  if (!p) return null;
  const last9 = p.slice(-9);
  const book = bookFor(dealerId, brokerId);
  const same = (v) => String(v || '').replace(/\D/g, '').slice(-9) === last9;
  /* An application stores the buyer inside the applicant json rather than in a column,
     so this is the one place the shape has to be opened. It is read-only and the phone
     is the only field touched - the identity documents in there are not the broker's
     business and never leave this function. */
  const appPhone = (row) => {
    try {
      return JSON.parse(row.applicant || '{}').phone || '';
    } catch {
      return '';
    }
  };
  ...
  return {
    phone: p,
    claim: book.clients.find((c) => c.phone === p) || null,
    checks: book.checks.filter((c) => same(c.phone)),
    leads: book.leads.filter((l) => same(l.phone)),
    applications: book.applications.filter((a) => same(appPhone(a))),
  };
}
...
module.exports = {
  registerClient, claimFor, confirmByClient, bookFor, historyFor, earningsFor, key, CLAIM_DAYS,
