...
/* The insurance panel.
 *
 * INVENTED, like the lenders. Every name, rate and commission below is a placeholder so
 * the quote engine has something to price - replace the whole list before a real
 * conversation, in Admin -> Insurers. The rates are in the right region for Kenyan motor
 * (roughly 3.5-5% of value a year, with a floor) so the numbers on screen are not silly,
 * but no insurer has agreed to any of them.
 *
 * They disagree with each other on purpose. A panel where everyone quotes the same thing
 * is a panel with no reason to exist, and the spread between cheapest and dearest is the
 * argument for using it.
 */
const INSURERS = [
  {
    name: 'Sentinel Assurance Kenya', short_name: 'Sentinel', logo_text: 'SA', color: '#1e5f3a',
    comprehensive_rate: 3.75, min_premium: 28000, third_party_premium: 7500,
    age_loading_from_years: 8, age_loading_pct: 0.15,
    ncd_pct_per_year: 10, ncd_max_pct: 30,
    excess_pct: 2.5, excess_min: 20000,
    max_vehicle_age_years: 15, commission_pct: 10,
    claim_days: 14, approved_garages: 180,
    highlights: JSON.stringify([
      'Claims settled in 14 days on average',
      '180 approved garages countrywide',
      'Free windscreen cover to KES 50,000',
    ]),
    excludes: JSON.stringify(['Wear and tear', 'Mechanical breakdown', 'Driving without a valid licence']),
    contact_name: 'Underwriting desk', contact_phone: '+254700000101',
    sort_order: 1,
  },
  {
    name: 'Harambee General Insurance', short_name: 'Harambee', logo_text: 'HG', color: '#8a4b1f',
    ...
    max_vehicle_age_years: 18, commission_pct: 10,
    claim_days: 21, approved_garages: 240,
    highlights: JSON.stringify([
      'Best no-claims discount on the panel - up to 40%',
      'Will write cover on cars up to 18 years old',
      'Towing included to 100km',
    ]),
    excludes: JSON.stringify(['Wear and tear', 'Tyre damage alone', 'Use as a taxi or for hire']),
    contact_name: 'Motor underwriting', contact_phone: '+254700000102',
    sort_order: 2,
  },
  {
    name: 'Tumaini Insurance Company', short_name: 'Tumaini', logo_text: 'TI', color: '#1e3a5f',
    ...
    max_vehicle_age_years: 10, min_sum_insured: 800000, commission_pct: 10,
    claim_days: 10, approved_garages: 90,
    highlights: JSON.stringify([
      'Cheapest rate on the panel for a newer car',
      'Claims settled in 10 days on average',
      'Courtesy car included on total loss',
    ]),
    excludes: JSON.stringify(['Cars over 10 years old', 'Vehicles under KES 800,000', 'Commercial use']),
    contact_name: 'Agency channel', contact_phone: '+254700000103',
    sort_order: 3,
  },
  {
    name: 'Mwangaza Microinsurance', short_name: 'Mwangaza', logo_text: 'MM', color: '#5f1e4a',
    comprehensive_rate: 4.75, min_premium: 18000, third_party_premium: 6900,
    age_loading_from_years: 10, age_loading_pct: 0.1,
    ncd_pct_per_year: 7.5, ncd_max_pct: 20,
    excess_pct: 5, excess_min: 10000,
    max_vehicle_age_years: 25, commission_pct: 10,
    claim_days: 30, approved_garages: 45,
    highlights: JSON.stringify([
      'Will cover an older car nobody else will touch',
      'Lowest minimum premium on the panel',
      'Premium payable in four instalments',
    ]),
    excludes: JSON.stringify(['Higher excess than the rest of the panel', 'Claims can take 30 days']),
    contact_name: 'Partnerships', contact_phone: '+254700000104',
    sort_order: 4,
  },
];
...
const STAFF = [
  ...
  /* dealer_id null on purpose. A broker is not staff at one yard - he walks into four of
     them in a week, and tying him to one would mean four accounts and four separate
     books. The permission set in auth.js is what keeps him out of the dealer's screens. */
  ['Peter Kariuki', 'peter@broker.demo', 'demo123', 'broker', null],
];
...
function seedIfEmpty() {
  ...
  INSURERS.forEach((i) => insert('insurers', i));
  ...
  /* WHERE THE BALANCE GOES. Published in the app on purpose.
     The commonest way a Kenyan car buyer is defrauded is being sent account details over
     WhatsApp by someone who is not the dealership. A buyer who can open the app and read
     the account from the same place their deal lives has one authoritative source, and
     "we never change these details by message" becomes a rule they can check. */
  setSetting('settle_bank_name', 'Demo Bank Kenya');
  setSetting('settle_account_name', DEALERS[0].name);
  setSetting('settle_account_no', '0100000000000');
  setSetting('settle_branch', 'Westlands');
  setSetting('settle_paybill', '000000');

  /* What this platform charges, and who for. Settings rather than constants so a second
     dealership can be on different terms without a code change. */
  setSetting('fee_band_1_max', 1500000);
  setSetting('fee_band_1', 2500);
  setSetting('fee_band_2_max', 3500000);
  setSetting('fee_band_2', 5000);
  setSetting('fee_band_3', 10000);
  setSetting('bank_fee_per_deal', 10000);
  setSetting('bank_fee_pct_of_loan', 0.5);

  /* Where this install actually lives. Used in the WhatsApp message a broker is sent
     when they are signed up in a field - a link they cannot open is worse than no link. */
  setSetting('public_url', 'https://motoke-demo.fly.dev');
...
/**
 * Add the demo broker to a database that was seeded before the role existed.
 *
 * seedIfEmpty only runs on a virgin database, so without this the broker console is
 * unreachable on every install that already has stock in it - including the one with the
 * generated showroom photographs, which nobody wants to throw away for a login.
 */
function ensureBrokerAccount() {
  const [name, email, password, role] = STAFF[STAFF.length - 1];
  if (role !== 'broker') return { added: false };
  /* Keyed on the EMAIL, not on whether ANY broker exists. A database that already had a
     broker in it from someone signing up through the public join form would otherwise
     suppress the demo account for good, and the login in the smoke suite would fail with
     a message about credentials rather than about the account never having been made. */
  if (get('SELECT id FROM users WHERE email=?', [email])) return { added: false };
  const { hash, salt } = hashPassword(password);
  insert('users', {
    name, email, role, password_hash: hash, salt, dealer_id: null,
    phone: '+254712004601',
  });
...
/**
 * Settings added after this database was seeded.
 *
 * Same reason as the broker account: seedIfEmpty only runs on a virgin database, and an
 * install with real stock in it would otherwise never learn about a new setting. Only
 * fills what is MISSING - a dealership that has typed its own bank details is never
 * overwritten with the demo ones.
 */
/**
 * The insurance panel, for a database seeded before insurers existed.
 *
 * Same reasoning as the broker account and the settlement details: seedIfEmpty only runs
 * on a virgin database, and an install with real stock in it would otherwise have an
 * insurance tab with nothing behind it. Skipped entirely the moment one insurer exists,
 * so a dealership that has typed in its own panel is never given four invented ones.
 */
function ensureInsurancePanel() {
  const existing = get('SELECT id FROM insurers LIMIT 1');
  if (existing) return { added: 0 };
  INSURERS.forEach((i) => insert('insurers', i));
  return { added: INSURERS.length };
}

/**
 * Give already-funded deals a funded_at, once.
 *
 * The column is new; the deals are not. Without this every statement for every past month
 * is empty and the invoicing screen looks broken on a database with two years of sales in
 * it. Dated from the disbursement event where there is one — the event trail knows when it
 * actually happened — and from updated_at where there is not, which is imperfect and is
 * the reason this runs once rather than continuously. New deals are stamped properly by
 * api.js at the moment the status changes.
 */
function backfillFundedAt() {
  const rows = all(
    "SELECT id, updated_at, created_at FROM applications WHERE funded_at IS NULL AND status IN ('disbursed','delivered','completed')"
  );
  let done = 0;
  for (const r of rows) {
    const ev = get(
      "SELECT created_at FROM application_events WHERE application_id=? AND message LIKE '%disbursed%' ORDER BY id LIMIT 1",
      [r.id]
    );
    const when = (ev && ev.created_at) || r.updated_at || r.created_at;
    if (!when) continue;
    run('UPDATE applications SET funded_at=? WHERE id=?', [new Date(String(when).replace(' ', 'T')).toISOString(), r.id]);
    done++;
  }
  return { dated: done };
}

function ensureSettlementDefaults() {
  const first = get('SELECT name FROM dealers ORDER BY id LIMIT 1') || {};
  const defaults = {
    public_url: 'https://motoke-demo.fly.dev',
    fee_band_1_max: 1500000,
    fee_band_1: 2500,
    fee_band_2_max: 3500000,
    fee_band_2: 5000,
    fee_band_3: 10000,
    bank_fee_per_deal: 10000,
    bank_fee_pct_of_loan: 0.5,
    settle_bank_name: 'Demo Bank Kenya',
    settle_account_name: first.name || 'The dealership',
    settle_account_no: '0100000000000',
    settle_branch: 'Westlands',
    settle_paybill: '000000',
  };
  let added = 0;
  for (const [k, v] of Object.entries(defaults)) {
    const row = get('SELECT value FROM settings WHERE key=?', [k]);
    if (!row) { setSetting(k, v); added++; }
  }
  return { added };
}

module.exports = { seedIfEmpty, ensureBrokerAccount, ensureSettlementDefaults, ensureInsurancePanel, backfillFundedAt, DEALERS, LENDERS, MEASURED, inspectionFor };
