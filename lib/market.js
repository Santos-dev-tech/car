...
/**
 * How the rest of the car moves the badge's number.
 *
 * The badge alone is too blunt: an E200 and an E200d are both a Mercedes, and they do not
 * hold value the same way. These adjust the make's retention for the things that actually
 * decide resale in Kenya.
 */

/* Body. A used buyer here wants ground clearance and the ability to carry people or
   goods. Saloons — especially big executive ones — have the smallest second-hand pool. */
const BODY_RETENTION = {
  SUV: 1.02, Pickup: 1.03, 'Station Wagon': 1.01, Hatchback: 1.0, MPV: 1.0,
  Van: 1.01, Sedan: 0.985, Coupe: 0.97, Truck: 1.02, Bus: 1.0,
};

/* Fuel. Diesel workhorses hold on; hybrids carry a battery-replacement worry that a
   Kenyan buyer prices in; electric has almost no second-hand market yet. */
const FUEL_RETENTION = { diesel: 1.015, petrol: 1.0, hybrid: 0.985, electric: 0.95 };

/**
 * What a car is likely to be worth after n more years, and what that costs per month.
 *
 * @param price      today's price
 * @param make       the badge — the strongest single signal
 * @param years      how far ahead
 * @param currentAge how old it is now
 * @param spec       { bodyType, fuel, engineCc } — optional, and worth passing:
 *                   without it every Mercedes gets the same curve regardless of whether
 *                   it is a 2.0 diesel saloon or a 4.0 petrol coupe.
 */
function resale(price, make, years = 3, currentAge = 0, spec = {}) {
  const base = RETENTION[make] || DEFAULT_RETENTION;

  const body = BODY_RETENTION[spec.bodyType] || 1;
  const fuel = FUEL_RETENTION[String(spec.fuel || '').toLowerCase()] || 1;

  /* Engine size. A big engine is a running cost in a country with 214-shilling petrol, so
     the pool of people who will buy it second-hand shrinks as the litres go up. Capped so
     a V8 is penalised, not written off. */
  const litres = (Number(spec.engineCc) || 0) / 1000;
  const engine = litres > 2.5 ? Math.max(0.965, 1 - (litres - 2.5) * 0.012) : 1;

  const adjusted = base * body * fuel * engine;

  /* Depreciation is steepest when a car is young, so the curve flattens with age.
     The easing moves the rate a FRACTION of the way toward the ceiling rather than
     adding a flat amount to it. Adding a flat +0.04 pushed every strong holder into the
     0.95 cap, which flattened a 1.5-litre Probox and a 4.5-litre V8 Land Cruiser to the
     same number — exactly the thing the variant adjustments above exist to prevent. */
  const CEILING = 0.95;
  const ageEase = Math.min(1, Math.max(0, currentAge) / 8) * 0.35;
  const eased = Math.min(CEILING, adjusted + (CEILING - adjusted) * ageEase);
  ...
  return {
    ...
    /* Shown so nobody has to take the number on trust, and so two cars of the same badge
       can be told apart on the page. */
    basis: {
      badge: +(base * 100).toFixed(1),
      bodyEffect: +((body - 1) * 100).toFixed(1),
      fuelEffect: +((fuel - 1) * 100).toFixed(1),
      engineEffect: +((engine - 1) * 100).toFixed(1),
      variantAware: !!(spec.bodyType || spec.fuel || spec.engineCc),
    },
  };
}
